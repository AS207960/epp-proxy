        ...
        T: Serialize,
{
    ...
    let out = format!("<?xml version=\"1.0\" encoding=\"UTF-8\"?><{} xmlns=\"{}\">{}</{}>", root, ns, val.as_str(), root);
    Ok(out)
}
    ...
    fn as_str(&self) -> String {
        match self {
            ...
            _SerializerData::Seq(s) => {
                s.into_iter().map(|d| d.as_str()).collect::<Vec<_>>().join("")
            }
        ...
        if key != "$value" {
            if let Some(ns) = caps.name("n") {
                if let Some(p) = caps.name("p") {
                    format!("<{}:{} xmlns:{}=\"{}\"{}>", p.as_str(), caps.name("e").unwrap().as_str(), p.as_str(), ns.as_str(), attrs)
                } else {
                   format!("<{} xmlns=\"{}\"{}>", caps.name("e").unwrap().as_str(), ns.as_str(), attrs)
                }
            } else {
                format!("<{}{}>", key, attrs)
            }
...
impl<'a> ser::SerializeStruct for StructSerializer<'a> {
            ...
            T: ?Sized + Serialize,
    {
        ...
        if key.starts_with("$attr:") {
            self.attrs.push(format!("{}=\"{}\"", &key[6..], val.as_str()))
        } else {
...
impl<'a> ser::SerializeStructVariant for StructVariantSerializer<'a> {
    ...
    fn end(self) ->  Result<_SerializerData, Self::Error> {
        Ok(_SerializerData::String(format_tag(&self.tag, &_SerializerData::Struct {
            attrs: self.attrs.join(" "),
            contents: self.keys.join("")
        })))
    }
