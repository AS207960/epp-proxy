        ...
        Some(value) => match value.value {
            proto::EPPResultDataValue::EPPHostInfoResult(host_info) => {
                Response::Ok(InfoResponse {
                    ...
                    addresses: host_info.addresses.into_iter().map(|a| Address {
                        address: a.address,
...
pub fn handle_create(client: &EPPClientServerFeatures, req: &CreateRequest) -> Result<proto::EPPCommandType, Response<CreateResponse>> {
    if !client.host_supported {
        return Err(Response::Unsupported)
    }
    if req.name.len() < 1 {
    ...
    let command = proto::EPPCreate::Host(proto::host::EPPHostCreate {
        name: req.name.clone(),
        addresses: match req.addresses.iter().map(|a| Ok(proto::host::EPPHostAddressSer {
...
pub fn handle_delete(client: &EPPClientServerFeatures, req: &DeleteRequest) -> Result<proto::EPPCommandType, Response<DeleteResponse>> {
    if !client.host_supported {
        return Err(Response::Unsupported)
    }
    if req.name.len() < 1 {
...
pub fn handle_update(client: &EPPClientServerFeatures, req: &UpdateRequest) ->  Result<proto::EPPCommandType, Response<UpdateResponse>> {
    if !client.host_supported {
        return Err(Response::Unsupported)
    }
    if req.name.len() < 1 {
    ...
    if req.add.len() < 1 && req.remove.len() < 1 && req.new_name.is_none() {
        return Err(Response::Err("at least one operation must be specified".to_string()))
    }
    match &req.new_name {
        Some(n) => {
            if n.len() < 1 {
                return Err(Response::Err("new host name has a min length of 1".to_string()))
            }
        },
        None => {}
    }
    let map_obj = |a: &UpdateObject| Ok(match a {
    ...
    let command = proto::EPPUpdate::Host(proto::host::EPPHostUpdate {
        ...
        add: match req.add.len() {
            ...
            _ => Some(proto::host::EPPHostUpdateAdd {
                params: req.add.iter().map(map_obj).collect::<Result<_, Response<UpdateResponse>>>()?
            })
        },
        remove: match req.remove.len() {
            0 => None,
            _ => Some(proto::host::EPPHostUpdateRemove {
                params: req.remove.iter().map(map_obj).collect::<Result<_, Response<UpdateResponse>>>()?
            })
        },
        change: req.new_name.as_ref().map(|n| proto::host::EPPHostUpdateChange {
            name: n.clone()
        })
    });
...
pub async fn check(host: &str, client_sender: &mut futures::channel::mpsc::Sender<Request>) -> Result<CheckResponse, super::Error> {
    let (sender, receiver) = futures::channel::oneshot::channel();
    super::send_epp_client_request(client_sender, Request::HostCheck(CheckRequest {
        name: host.to_string(),
        return_path: sender
    }), receiver).await
}

pub async fn info(host: &str, client_sender: &mut futures::channel::mpsc::Sender<Request>) -> Result<InfoResponse, super::Error> {
    let (sender, receiver) = futures::channel::oneshot::channel();
    super::send_epp_client_request(client_sender, Request::HostInfo(InfoRequest {
        name: host.to_string(),
        return_path: sender
    }), receiver).await
}
