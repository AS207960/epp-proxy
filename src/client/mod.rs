...
async fn recv_msg<R: std::marker::Unpin + tokio::io::AsyncRead>(sock: &mut R, host: &str) -> Result<proto::EPPMessage, ()> {
...
impl EPPClientReceiver {
    fn run(mut self) -> futures::channel::mpsc::Receiver<Result<proto::EPPMessage, ()>> {
        let (mut sender, receiver) = futures::channel::mpsc::channel::<Result<proto::EPPMessage, ()>>(16);
...
impl EPPClient {
        ...
        match res.message {
            proto::EPPMessageType::Response(response) => {
                if !response.is_success() {
                    warn!("Received failure result from {}: {}", self.server_id, response.response_msg());
                }
                let transaction_id = match &response.transaction_id.client_transaction_id {
                    Some(i) => i,
                    None => {
                        error!("Received response without client transaction ID from {}", self.server_id);
                        return Err(())
                    }
                };
                let transaction_id = match uuid::Uuid::parse_str(&transaction_id) {
                    Ok(i) => i,
                    Err(e) => {
                        error!("Received response with invalid transaction UUID from {}: {}", self.server_id, e);
                        return Err(())
                    }
                };
            ...
            o => {
                warn!("Received unexpected response from {}: {:?}", self.server_id, o);
                Ok(())
            }
        }
    }

    async fn _setup_connection(&mut self, sock: &mut tokio_tls::TlsStream<TcpStream>) -> Result<(), bool> {
        ...
        if let proto::EPPMessageType::Greeting(greeting) = msg.message {
            ...
            error!("Didn't receive greeting as first message from {}", &self.host);
            info!("Restarting connection...");
            self._close(sock).await;
            return Err(false);
        };
    ...
    async fn _process_greeting(&mut self, greeting: proto::EPPGreeting) -> Result<(), ()> {
        ...
        if (greeting.server_date - Utc::now()).num_minutes() >= 5 {
            warn!("Local time out by more than 5 minutes from time reported by {}", greeting.server_id);
        }
        self.features.contact_supported = greeting.service_menu.objects.iter().any(|e| e == "urn:ietf:params:xml:ns:contact-1.0");
        self.features.domain_supported = greeting.service_menu.objects.iter().any(|e| e == "urn:ietf:params:xml:ns:domain-1.0");
        self.features.host_supported = greeting.service_menu.objects.iter().any(|e| e == "urn:ietf:params:xml:ns:host-1.0");

        if !(self.features.contact_supported | self.features.domain_supported | self.features.host_supported) {
    ...
    async fn _login(&self, sock: &mut tokio_tls::TlsStream<TcpStream>) -> Result<(), ()> {
        ...
        match self._send_command(proto::EPPCommandType::Login(login_command), sock, None).await {
            Ok(i) => i,
            Err(_) => {
        ...
        if let proto::EPPMessageType::Response(response) = msg.message {
            if !response.is_success() {
                error!("Login to {} failed with error: {}", self.server_id, response.response_msg());
                Err(())
            } else {
            ...
            error!("Didn't receive response to login command from {}", self.server_id);
            Err(())
        }
    }

    async fn _send_command<W: std::marker::Unpin + tokio::io::AsyncWrite, M: Into<Option<uuid::Uuid>>>
    (&self, command: proto::EPPCommandType, sock: &mut W, message_id: M) -> Result<uuid::Uuid, ()> {
    ...
    async fn _send_msg<W: std::marker::Unpin + tokio::io::AsyncWrite>(&self, message: &proto::EPPMessage, sock: &mut W) -> Result<(), ()> {
        let encoded_msg = xml_ser::to_string( message, "epp", "urn:ietf:params:xml:ns:epp-1.0").unwrap();
    ...
    async fn _close(&mut self, sock: &mut tokio_tls::TlsStream<TcpStream>) {
        ...
        match sock.shutdown().await {
            ...
            Err(err) => {
                error!("Error closing connection to {}: {}, dropping anyway", &self.host, err);
            }
        }
    }
...
async fn send_epp_client_request<R>(
    client_sender: &mut futures::channel::mpsc::Sender<Request>, req: Request,
